# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90" 2

# 1 "./params.h" 1
# 3 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90" 2
...
# 72 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90" 2
...
# 73 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90" 2
...
! stratiform precipitation variables   !! sxj---
   ...
   integer ifld,itim
    ...
    ! added by SHI Xiangjun for MG microphysics
    integer ix_cldice, ix_cldliq
    logical, parameter :: indirect_aerosol = .false.
    logical, parameter :: detrain_to_cld = .true.
...
# 568 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90"
                 !
    ...
    else if (convection_scheme == 'Zhang_Neale') then   !20101116ljliaccording to SXJ
        call t_startf ('zm_convr')
...
# 754 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90"
!
! put the detraining cloud water into the cloud and environment in
! proportion to the cloud fraction
!
...
# 946 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/tphysbc.F90"
!
! put the detraining cloud water into the cloud and environment in
! proportion to the cloud fraction
!
...
!-----------------------------------------------------------XXX
    ...
    if (RK_or_MG=='RK') then
        ...
        ! stratiform condensation via prognostic cloud water calculate tendencies
        call t_startf('pcond')
        zero(:ncol,:pverp) = 0.0
        ...
        ! Li Dong: What do you mean "interactive"?
        ! make it interactive
        do k = 1, pver
            do i = 1, ncol
...
!!(2004)
    ...
    if (dosw .or. dolw) then
        ...
        if (RK_or_MG == 'MG' .and. indirect_aerosol) then
            ...
            do k = 1, pver
                do i = 1, ncol
                    rel(i,k) = max(rel(i,k), 10.0)
                    rel(i,k) = min(rel(i,k), 20.0)
                    rei(i,k) = max(rei(i,k), 10.0)
                    rei(i,k) = min(rei(i,k), 30.0)
                end do
            end do
        ...
        if (RK_or_MG == 'RK') then
            do k = 1, pver
                do i = 1, ncol
                    ...
                    icwmr(i,k) = state%q(i,k,ixcldw)*(1.0-fice(i,k))/max(0.01_r8, cldn(i,k))
                end do
            end do
        ...
        ! radctl can change pmxrgn and nmxrgn so cldsav needs to follow radctl.
        !
