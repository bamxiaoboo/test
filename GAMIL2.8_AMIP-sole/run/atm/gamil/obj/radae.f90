# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90" 2

# 1 "./params.h" 1
# 3 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90" 2
module radae
!------------------------------------------------------------------------------
!
! Description:
...
! Author:  B. Collins
!
! $Id: radae.F90,v 1.10.4.7 2002/09/20 20:47:39 rosinski Exp $
!------------------------------------------------------------------------------
...
!
! Collins/Hackney/Edwards (C/H/E) H2O parameterization
!
! Notation:
...
# 243 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90" 2
!------------------------------Arguments--------------------------------
!
! Input arguments
!
...
!
! Variables for Collins/Hackney/Edwards H2O parameterization
!
! Notation:
! U   = integral (P/P_0 dW)  eq. 15 in Ramanathan/Downey 1986
! P   = atmospheric pressure
...
!
   do 200 k1=pverp,ntoplw,-1
      do 100 k2=pverp,ntoplw,-1
         if (k1 == k2) go to 100
         do i=1,ncol
...
!
            ...
            fwku(i) = fwk(i)*u(i)
!
! Define variables for C/H/E fit
!
! abso(i,1)     0 -  800 cm-1   h2o rotation band
! abso(i,1)  1200 - 2200 cm-1   h2o vibration-rotation band
! abso(i,2)   800 - 1200 cm-1   h2o window
...
!
            ...
            abso(i,ib) = min(max(fa * a_star, 0.0_r8), 1.0_r8)
!
...
!
            ...
            abso(i,ib) = min(max(fa * (1.0 - l_star * c_star), 0.0_r8), 1.0_r8) 
!
! Invoke linear limit for scaling wrt u below min_u_h2o
!
            if (uvar < min_u_h2o) then
...
# 1597 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90" 2
!------------------------------Arguments--------------------------------
!
! Input arguments
!
...
!
         ...
         emis(i,ib) = min(max(fe * e_star, 0.0_r8), 1.0_r8)
!
! Invoke linear limit for scaling wrt u below min_u_h2o
!
         if (uvar < min_u_h2o) then
...
!
         ...
         emis(i,ib) = min(max(fe * (1.0 - l_star * c_star), 0.0_r8), 1.0_r8)
!
! Invoke linear limit for scaling wrt u below min_u_h2o
!
         if (uvar < min_u_h2o) then
...
# 2475 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/radae.F90" 2
...
!+WDC 11/29/00
!
!     Read in the absorptivity and emissivity for Collins/Hackney/Edwards 
!          method
!
   if ( masterproc ) then
      ...
      if (psize    /= n_p  .or. &
          ...
          tesize   /= n_te .or. &
          rhsize   /= n_rh) then
         write(6,*) 'RADINI: dimensions for abs/ems do not match internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: non-wndw abs. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: non-wndw ems. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: window abs. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: cont. trans for abs. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: cont. trans. for ems. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: line trans for abs. in file /= internal def.'
         call endrun
      endif
      ...
      if (ndims /= 5 .or. &
           ...
           dims(4) /= tedimid   .or. &
           dims(5) /= rhdimid) then
         write(6,*) 'RADINI: line trans. for ems. in file /= internal def.'
         call endrun
      endif
...
!
! H2O Continuum: CKD 2.1
! Code for continuum: GENLN2, version 4
...
!     where <> indicates average over wavenumber, is 2.08
! 
...
! Line data for H2O: Hitran 1996 
! Isotopes of H2O: all
! Line widths: air-broadened only (self set to 0)
! Code for line strengths and widths: GENLN2, version 4
! Reference: Edwards, D.P., 1992: GENLN2, A General Line-by-Line Atmospheric
!                     Transmittance and Radiance Model, Version 3.0 Description
!                     and Users Guide, NCAR/TN-367+STR, 147 pp.
!
...
!
   ...
   real(r8),parameter ::  phi_r0(nbands) = (/   9.60901515E-01,  -19.345926 /)
   real(r8),parameter ::  phi_r1(nbands) = (/   4.83354282E-04,    0.38230940 /)
   real(r8),parameter ::  phi_r2(nbands) = (/  -1.82756255E-06,   -0.0027408001 /)
   real(r8),parameter ::  phi_r3(nbands) = (/   2.09446996E-09,    7.1454173e-06 /)
...
!
! Short function for Hulst-Curtis-Godson temperature factors for
!   computing effective pressure along path
! Line data for H2O: Hitran 1996 
! Isotopes of H2O: all
! Line widths: air-broadened only (self set to 0)
! Code for line strengths and widths: GENLN2, version 4
...
!
   ...
   real(r8),parameter ::  psi_r0(nbands) = (/   5.58938667E-01, -45.536981 /)
   real(r8),parameter ::  psi_r1(nbands) = (/   4.03807913E-03,   0.82249547 /)
   real(r8),parameter ::  psi_r2(nbands) = (/  -9.78672419E-06,  -0.0052056773 /)
   real(r8),parameter ::  psi_r3(nbands) = (/   1.13294353E-08,   1.1746943e-05 /)
