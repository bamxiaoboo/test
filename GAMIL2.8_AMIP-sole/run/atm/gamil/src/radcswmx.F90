...
#ifdef COSP
                if (ns == 1) then
                    ! output optical depth of cloud at 0.67 micron for COSP
                    ! added by Li Dong
                    ! TODO: Should we split the liquid and ice tau here?
                    call outfld("TAU", tauxcl(:,1:pver)+tauxci(:,1:pver), pcols, lchnk)
                end if
#endif
