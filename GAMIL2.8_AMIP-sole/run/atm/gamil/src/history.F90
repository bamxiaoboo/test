...
!
        ...
        ! add COSP variables
#ifdef COSP
        call addfld("PMID    ", "Pa      ", pver, "A", "Pressure at middle level", phys_decomp)
        call addfld("PINT    ", "Pa      ", pver, "A", "Pressure at interface level", phys_decomp)
        call addfld("ZMID    ", "m       ", pver, "A", "Height at middle level", phys_decomp)
        call addfld("ZINT    ", "m       ", pver, "A", "Height at interface level", phys_decomp)
        call addfld("LSRAIN  ", "kg m-2 s-1", pver, "A", "Stratiform rain flux", phys_decomp)
        call addfld("LSSNOW  ", "kg m-2 s-1", pver, "A", "Stratiform snow flux", phys_decomp)
        call addfld("CCRAIN  ", "kg m-2 s-1", pver, "A", "Convective rain flux", phys_decomp)
        call addfld("CCSNOW  ", "kg m-2 s-1", pver, "A", "Convective snow flux", phys_decomp)
        call addfld("O3MMR   ", "kg kg-1 ", pver, "A", "Ozone mass mixing ratio", phys_decomp)
        call addfld("REFFLIQ ", "micron  ", pver, "A", "Effective radius of liquid hydrometeor", phys_decomp)
        call addfld("REFFICE ", "micron  ", pver, "A", "Effective radius of ice hydrometeor", phys_decomp)
        call addfld("EMIS    ", "1       ", pver, "A", "Long wave emissivity at 10.5 micron", phys_decomp)
        call addfld("SUNLIT  ", "1       ", 1, "I", "Day points", phys_decomp)
        call addfld("TAU     ", "m       ", pver, "A", "Optical depth of cloud at 0.67 micron", phys_decomp)
#endif
!
