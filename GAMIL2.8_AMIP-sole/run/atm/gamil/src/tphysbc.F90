...
#ifdef COSP
    ! added by DONG Li for COSP
    real(8) ccrain(pcols,pver), ccsnow(pcols,pver)
    real(8) sunlit(pcols)
#endif
...
#ifdef COSP
                  lengath, icwmr1, ptendlaq, ptendlat, ccrain, ccsnow)
#else
...
#ifndef PCWDETRAIN
                 !
                 ! put the detraining cloud water into precip to conserve mass
                 !
                 do k = 1, pver
                     do i = 1, ncol
...
#ifdef COSP
        ccsnow = 0.1_r8*ccsnow   !!!units changes:mm/s to kg.m^2/s!!!
                                     !!!snow density is 100 kg/m^3!!!
        call outfld("CCRAIN  ", ccrain, pcols, lchnk)
        call outfld("CCSNOW  ", ccsnow, pcols, lchnk)
#endif
...
#ifdef COSP
    sunlit = merge(1.0_r8, 0.0_r8, coszrs > 0.0_r8)
    call outfld("SUNLIT  ", sunlit, pcols, lchnk)
#endif
!
...
#ifdef COSP
        call outfld("REFFLIQ ", rel, pcols, lchnk)
        call outfld("REFFICE ", rei, pcols, lchnk)
