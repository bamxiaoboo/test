# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/lnd/clm2/src/biogeophys/Biogeophysics_Lake.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/lnd/clm2/src/biogeophys/Biogeophysics_Lake.F90"

# 1 "./misc.h" 1
# 2 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/lnd/clm2/src/biogeophys/Biogeophysics_Lake.F90" 2
...
# 3 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/lnd/clm2/src/biogeophys/Biogeophysics_Lake.F90" 2
...
!
!-----------------------------------------------------------------------
! $Id: Biogeophysics_Lake.F90,v 1.6.6.3 2002/06/15 13:50:13 erik Exp $
!-----------------------------------------------------------------------
...
!----Local Variables----------------------------------------------------
  ...
  real(r8) ax      ! temporary variable
  real(r8) bx      ! temporary variable
  real(r8) beta1   ! coefficient of convective velocity [-]
  ...
  real(r8) dzsur   ! thickness of snow cover plus top lake layer [m]
  real(r8) eg      ! water vapor pressure at temperature T [pa]
  real(r8) emg     ! ground emissivity
  real(r8) hm      ! energy residual [W/m2]
  real(r8) htvp    ! latent heat of vaporization (or sublimation) [J/kg]
  real(r8) obu     ! monin-obukhov length [m]
  real(r8) obuold  ! monin-obukhov length of previous iteration [m]
  ...
  real(r8) stftg3  ! temporary variable
  real(r8) temp1   ! relation for potential temperature profile
  real(r8) temp2   ! relation for specific humidity profile
  real(r8) tgbef   ! lake surface temperature from previous iteration [K]
  real(r8) thm     ! intermediate variable (forc_t+0.0098*forc_hgt_t) [K]
  real(r8) thv     ! virtual potential temperature [K]
  real(r8) thvstar ! virtual potential temperature scaling parameter [K]
  real(r8) tksur   ! thermal conductivity of snow/soil [W/m/K]
  ...
  real(r8) beta(2) ! fraction solar rad absorbed at surface:
                   !  depends on lake type
  real(r8) za(2)   ! base of surface absorption layer (m): depends on lake type
  real(r8) eta(2)  ! light extinction coefficient (m-1): depends on lake type
  ...
  real(r8) rhow(nlevlak) ! density of water [kg/m3]
  real(r8) phi(nlevlak)  ! solar radiation absorbed by layer [W/m2]
  real(r8) kme(nlevlak)  ! molecular + eddy diffusion coefficient [m2/s]
  real(r8) cwat    ! specific heat capacity of water [J/m3/K]
  ...
  real(r8) fin     ! heat flux into lake - flux out of lake [W/m2]
  ...
  real(r8) ke      ! eddy diffusion coefficient [m2/s]
  real(r8) km      ! molecular diffusion coefficient [m2/s]
  real(r8) zin     ! depth at top of layer [m]
  real(r8) zout    ! depth at bottom of layer [m]
  real(r8) drhodz  ! d [rhow] /dz [kg/m4]
...
!
  ...
  call QSat(clm%t_grnd, clm%forc_pbot, eg, degdT, qsatg, &
            qsatgdT     )
...
! using lake surface temperature from previous time step
!
...
!
  ...
  if (clm%h2osno > 0.5 .AND. clm%t_grnd > tfrz) then
     ...
     clm%qflx_evap_soi = clm%forc_rho*(qsatg+qsatgdT*(clm%t_grnd-tgbef) & 
                       - clm%forc_q)/raw  !note: qsatg and qsatgdT should be f(tgbef)
  endif
...
!
  ...
  if (clm%h2osno > 0. .AND. clm%t_grnd >= tfrz) then
     hm = min( clm%h2osno*hfus/clm%dtime, max(clm%eflx_soil_grnd,0._r8) )
  else
     hm = 0.
  endif
...
!
  ...
  do j = 1, nlevlak-1
     ...
     den = max( (ws**2) * exp(-2.*ks*clm%z(j)), 1.e-10_r8 )
