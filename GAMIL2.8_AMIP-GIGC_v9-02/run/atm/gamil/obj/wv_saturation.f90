# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/wv_saturation.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/physics/cam1/wv_saturation.F90"
!
...
!
! $Id: wv_saturation.F90,v 1.1.4.2 2002/06/15 13:49:30 erik Exp $
!
module wv_saturation
    ...
    public aqsatd   ! Same as aqsat, but also returns a temperature derivative
    public vqsatd   ! Vector version of aqsatd
    public calc_qs  ! added by DONG Li for calculating 2m relative humidity
    !
    ! Data used by cldwat
    !
...
contains
    ...
    real(r8) function estblf(td)
        ...
        real(r8) e                 ! intermediate variable for es look-up
                                   ! partial pressure statement function
        e(td) = max(min(td, tmax), tmin)
        ...
        estblf = (tmin+int(e(td)-tmin)-e(td)+1.0)* &
                 estbl(int(e(td)-tmin)+1)-(tmin+int(e(td)-tmin)-e(td))* &
                 estbl(int(e(td)-tmin)+2)

    end function estblf

    subroutine gestbl(tmn, tmx, trice, ip, epsil, latvap, latice, rh2o, cpair, tmeltx)
        !-----------------------------------------------------------------------
        !
        ! Purpose:
        ...
        if (lentbl > plenest) then
            write(*, "('Error: wv_saturation::gestbl: ')", advance="no")
            write(*, "('The requested temperature range ')", advance="no") 
            write(*, "('(tmin, tmax) is larger than the fixed ""plenest""')")
            write(*, "('  tmin    = ', F)") tmin
            write(*, "('  tmax    = ', F)") tmax
            write(*, "('  plenest = ', F)") plenest
            call endrun
        end if
        !
        ...
        !
        if (masterproc) then
            write(*, "('Notice: wv_saturation::gestbl: Finished.')")
        end if
