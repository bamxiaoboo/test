# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/control/interpolate_data.F90"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/data3/work/yuxinzhu/test/model_platform/models/atm/GAMIL2.8_AMIP/src/control/interpolate_data.F90"
...
!
    ...
    if(extrap_method.eq.0) then
       ...
       if (ratio < 0.9 .or. ratio > 1.1) then
          write(6,*) 'Lininterp: Bad dyinwrap value =',dyinwrap,&
    ...
    if (extrap.gt.30. .and. .not. single_column) then
       write(6,*) 'interpolate_data:','yout=',minval(yout),maxval(yout),increasing,nout
       write(6,*) 'interpolate_data:','yin=',yin(1),yin(nin)
       write(6,*) 'interpolate_data:',extrap,' % of output grid will have to be extrapolated'
       call endrun('interpolate_data: ')
    end if
